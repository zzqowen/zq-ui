<template>
  <div
    class="zq-slider"
    :class="{ 'is-vertical': vertical, 'zq-slider--with-input': showInput }"
    role="slider"
    :aria-valuemin="min"
    :aria-valuemax="max"
  ...
  >
    ...
    ></zq-input-number>
    <div
      class="zq-slider__runway"
      :class="{ 'show-input': showInput, 'disabled': sliderDisabled }"
      :style="runwayStyle"
      @click="onSliderClick"
      ref="slider"
    >
      <div class="zq-slider__bar" :style="barStyle"></div>
      <slider-button
        :vertical="vertical"
        v-model="firstValue"
      ...
      <template v-for="(item, key) in stops">
        <div class="zq-slider__stop" :key="key" :style="getStopStyle(item)" v-if="showStops"></div>
      </template>
      <template v-if="markList.length > 0">
        <div>
          <div
            ...
            class="zq-slider__stop zq-slider__marks-stop"
            :key="key"
          ></div>
        </div>
        <div class="zq-slider__marks">
          <slider-marker
            :mark="item.mark"
            v-for="(item, key) in markList"
            :key="key"
...
import ZqInputNumber from "zq-ui/packages/input-number";
      ...
      if (this.range && Array.isArray(val)) {
        if (val[1] < this.min) {
          ...
          if (this.valueChanged()) {
            this.dispatch("ZqFormItem", "zq.form.change", [
              this.minValue,
              this.maxValue,
            ]);
