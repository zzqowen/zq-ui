import fecha from 'zq-ui/zq-ui-src/utils/date';
import { t } from 'zq-ui/zq-ui-src/locale';
...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`zq.datepicker.weeks.${ week }`)),
    dayNames: weeks.map(week => t(`zq.datepicker.weeks.${ week }`)),
    monthNamesShort: months.map(month => t(`zq.datepicker.months.${ month }`)),
    monthNames: months.map((month, index) => t(`zq.datepicker.month${ index + 1 }`)),
    amPm: ['am', 'pm']
  };
};
